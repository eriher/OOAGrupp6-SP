...
public class JSchedule extends JPanel implements Observer {
	...
	public void update(Observable o, Object arg) {
				fillPanels((ScheduleHandler)arg);
	}
	...
	public void initBuild() {
		...
		JLabel displayWeek = createJLabel("Vecka 9"); // STATISKT VÄRDE ATM
														// ÄNDRA!
	 ...
	 */
	public void fillPanels(ScheduleHandler handler) {

		removeAll();

		ArrayList<Integer> checkIn = null; // Beh�ver separata listor f�r att
											// f�renkla loopar.
		...
		/*
		 * //TESTV�RDEN START, ScheduleHandler ger f�r tillf�llet bara 1 par
		 * tider, vill pr�va med minst 2. ArrayList<Integer> checkInList = new
		 * ArrayList<Integer>(); checkInList.add(60); checkInList.add(140);
		 * ArrayList<Integer> checkOutList = new ArrayList<Integer>();
		 * checkOutList.add(100); checkOutList.add(180);
		 * 
		 * ArrayList<Integer> scheduledTimeList = new ArrayList<Integer>();
		 * scheduledTimeList.add(20); scheduledTimeList.add(80);
		 * scheduledTimeList.add(120); scheduledTimeList.add(160);
		 * scheduledTimeList.add(180); scheduledTimeList.add(200);
		 * 
		 * // ArrayList<ArrayList<Integer>> totalList = new
		 * ArrayList<ArrayList<Integer>>(3);
		 * 
		 * totalList.add(checkInList); 
		 * totalList.add(checkOutList);
		 * totalList.add(scheduledTimeList); //TESTV�RDEN SLUT
		 * --------------------------------------------------------------------
		 */

		System.out.println("inne i fillpanels");
		...
		for (int i = 0; i <= 6; i++) {
			...
			if (checkIn.size() == checkOut.size()) {
				...
				System.out
						.println("Error: size of checkIn does not match the size of checkOut.");
